...
async def pub_(bot, message):
    ...
    i = sts.get(full=True)
    bot_id = i.bot['id']
    if temp.lock.get(bot_id) and str(temp.lock.get(bot_id)) == "True":
        return await message.answer(f"Bot {i.bot['name']} is already in use. Wait until its task completes or use another bot.", show_alert=True)
    ...
    try:
        ...
        await msg_edit(m, f"**Source chat may be private. Use a userbot (must be a member) or make your [Bot](t.me/{_bot['username']}) an admin there**", retry_btn(frwd_id), True)
    ...
    await send(client, user, f"<b>Forwarding started with {_bot['name']} <a href=https://t.me/dev_gagan>Dev Gagan</a></b>")
    sts.add(time=True)
    sleep = 1 if _bot['is_bot'] else 10
    await msg_edit(m, "<code>Processing...</code>")
    temp.IS_FRWD_CHAT.append(i.TO)
    temp.lock[bot_id] = locked = True
    if locked:
        try:
            MSG = []
            pling = 0
            ...
            return await stop(client, user)
        temp.IS_FRWD_CHAT.remove(sts.get('TO'))
        await send(client, user, f"<b>🎉 Forwarding completed with {_bot['name']} 🥀 <a href=https://t.me/dev_gagan>SUPPORT</a>🥀</b>")
...
async def stop(client, user):
    ...
    temp.forwardings -= 1
    bot_id = sts.get('bot')['id']
    temp.lock[bot_id] = False
